...
//});
//#endregion
//var defaultCultureName = app.Configuration["Localization:DefaultCulture"] ?? "en-IN";
//var supportedCultureNames = app.Configuration.GetSection("Localization:SupportedCultures").Get<string[]>()
//                             ?? new[] { "en-IN" };
...
builder.Services.AddSwaggerGen(c =>
{
    ...
    c.SwaggerDoc("FinanceManagement", new OpenApiInfo { Title = "Fee Management API", Version = "v1" });
    c.SwaggerDoc("Login", new OpenApiInfo { Title = "Login API", Version = "v1" });
...
builder.Services.AddRateLimiter(options =>
{
    ...
    {
        opt.PermitLimit = Convert.ToInt32(builder.Configuration["V3MLoginAPI_Call_Attempt"]);
        opt.Window = TimeSpan.FromSeconds(Convert.ToInt32(builder.Configuration["V3MLoginAPI_Call_InDuration"]));
        opt.QueueProcessingOrder = QueueProcessingOrder.OldestFirst;
        opt.QueueLimit = 0;
    });
    ...
    {
        opt.PermitLimit = Convert.ToInt32(builder.Configuration["V3MAPI_Call_Limit_Attempt"]);
        opt.Window = TimeSpan.FromSeconds(Convert.ToInt32(builder.Configuration["V3MAPI_Call_Limit_InDuration"]));
    });
...
builder.Services.AddHttpClient();
builder.Services.AddScoped<AuthenticationStateProvider, ServerAuthenticationStateProvider>();
builder.Services.Configure<Infrastructure.User.SmtpSettings>(builder.Configuration.GetSection("Smtp"));
#endregion
