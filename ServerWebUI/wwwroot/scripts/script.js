...
window.menuMap = function () {
    ...
    $(document).on('click', '.open-menu', function (e) {
        ...
        // Left +/- icon
        let $toggleIcon = $li.children('.branch').find('.toggle-icon').first();

        // Folder icon
        let $folderIcon = $li.children('.branch').find('.folder-icon').first();

        // Right side Bootstrap icon (if present)
        let $rightIcon = $li.children('.branch').children('.bi');

        if ($nextTree.is(':visible')) {

            // Close child trees
            $nextTree.find('ul.tree').slideUp(200).removeClass('d-flex');
